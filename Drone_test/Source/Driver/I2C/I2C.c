...
void I2C_Init(I2C_h *handler, I2C_Channel I2C_Ch_Index, uint32_t I2C_Freq_KHz)
{
	...
	I2C_config[I2C_Index].GPIO_Perif_Addr->PCTL       |=   ((I2C_config[I2C_Index].GPIOPCTL_Index<<((I2C_config[I2C_Index].SCL_Pin)*4)) |
	                                                       (I2C_config[I2C_Index].GPIOPCTL_Index<<((I2C_config[I2C_Index].SDA_Pin)*4)));                             //CONIGURING PORT A PIN 6 AND PIN 7 AS I2C
	...
	*/
    I2C_TPR = (SYS_CLOCK_HZ / (2 * (6 + 4) * I2C_Freq_KHz*1000)) - 1;

	I2C_config[I2C_Index].I2C_Perif_Addr->MTPR        |=   0x7F & I2C_TPR; 
	
	handler->i2c_ch_idx = I2C_Index;
	handler->i2c_freq   = I2C_Freq_KHz;
	handler->i2c_init   = 1;
}
/*****************************************************************************************************************************************/
...
void I2C_Write_Memory(I2C_h handler, uint8_t slave_address , uint8_t slave_memory_address , uint8_t data)
{
    ...
    I2C_Write_Memory_Multiple(handler, slave_address, slave_memory_address, &data, 1);
}
...
int I2C_Read(I2C_h handler, uint8_t slave_address)
{
	...
	I2C_Read_Multiple(handler, slave_address , temp_data  , 1);
	return (*temp_data);
}
/*****************************************************************************************************************************************/
...
int I2C_Read_Memory(I2C_h handler, uint8_t slave_address , uint8_t slave_memory_address)
{
	...
	I2C_Write(handler, slave_address , slave_memory_address);
	
	I2C_recieved_data = I2C_Read(handler, slave_address);
...
void I2C_Read_Memory_Multiple(I2C_h handler, uint8_t slave_address , uint8_t slave_memory_address , uint8_t *data , uint32_t num_of_data)
{
	...
	I2C_Read_Multiple(handler, slave_address ,  data ,  num_of_data);
}
/*****************************************************************************************************************************************/
